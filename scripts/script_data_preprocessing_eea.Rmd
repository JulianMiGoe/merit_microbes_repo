...
# Ensure all variables are present and use only column names in mutate
# Remove df_eea_2_merged$ prefix inside mutate

df_eea_2_merged <- df_eea_2_merged %>%
  mutate(
    Inc_start = as.POSIXct(Inc_start, format="%H:%M"),
    Inc_end = as.POSIXct(Inc_end, format="%H:%M"),
    Inc_time = 24 + as.numeric(difftime(Inc_end, Inc_start, units="hours")),
    Total_g = EEA_g + EEA_buffer,
    Quench_coeff = (Quench - Hom_ctrl) / Standard,
    Emission_coeff = Standard / n,
    Net_fluorescence = ((Assay - Hom_ctrl) / Quench_coeff) - Sub_ctrl,
    Activity_OM = ifelse(
      is.na(Net_fluorescence) | is.na(EEA_buffer) | is.na(OM_g) | is.na(Inc_time) | is.na(Emission_coeff) |
      OM_g == 0 | Inc_time == 0 | Emission_coeff == 0,
      NA,
      (Net_fluorescence * EEA_buffer) / (OM_g * Inc_time * Emission_coeff * 0.2)
    )
  )
...
df_glu_data <- df_eea_3_filtered %>%
  filter(Enzyme == "GLU") %>%
  dplyr::select(ID.y, GLU_value = Activity_OM, Depth.x, Zone, Treatment, Layer)  

df_chi_data <- df_eea_3_filtered %>%
  filter(Enzyme == "CHI") %>%
  dplyr::select(ID.y, CHI_value = Activity_OM, Depth.x, Zone, Treatment, Layer) 

df_leu_data <- df_eea_3_filtered %>%
  filter(Enzyme == "LEU") %>%
  dplyr::select(ID.y, LEU_value = Activity_OM, Depth.x, Zone, Treatment, Layer) 
