...
# Robust mixed model (alternative approach)
stat_model_alpha_robust_1_alternative <- rlmer(Value ~ Treatment * Zone * Layer + (1|Plot), 
                                              data = df_alpha_div_3_long)

# Pairwise comparisons for robust model
stat_alpha_zone_emm_robust_1_alternative <- emmeans(stat_model_alpha_robust_1_alternative, ~ Zone)
stat_alpha_zone_pairs_robust_1_alternative <- pairs(stat_alpha_zone_emm_robust_1_alternative)

stat_alpha_treatment_emm_robust_1_alternative <- emmeans(stat_model_alpha_robust_1_alternative, ~ Treatment)
stat_alpha_treatment_pairs_robust_1_alternative <- pairs(stat_alpha_treatment_emm_robust_1_alternative)

stat_alpha_layer_emm_robust_1_alternative <- emmeans(stat_model_alpha_robust_1_alternative, ~ Layer)
stat_alpha_layer_pairs_robust_1_alternative <- pairs(stat_alpha_layer_emm_robust_1_alternative)
```
...
df_elevation_delta_1_processed <- df_plot_pairs_elevation_1_processed %>%
  ...
  select(plotid1, plotid2, delta_elevation)
...
## Step 6d: Merge environmental data with community dissimilarities
```{r}
...
df_bray_delta_topsoil_1_processed <- df_bray_curtis_topsoil_2_merged %>%
  ...
  select(sample_name, comparison, plotid1, plotid2, BrayCurtisDissimilarity, 
...
df_bray_delta_topsoil_elevation_1_processed <- df_bray_curtis_topsoil_2_merged %>%
  ...
  select(sample_name, comparison, plotid1, plotid2, BrayCurtisDissimilarity, delta_elevation, zone1, zone2) %>%
  rowwise() %>%
  mutate(sorted_plotids = paste(sort(c(plotid1, plotid2)), collapse = "_")) %>%
  ungroup() %>%
  distinct(sorted_plotids, BrayCurtisDissimilarity, .keep_all = TRUE) %>%
...
## Step 6e: Environmental dissimilarity plots and statistics
```{r}
...
# Elevation-dissimilarity relationships
stat_lm_topsoil_elevation_1_main <- lm(BrayCurtisDissimilarity ~ delta_elevation, 
                                       data = df_bray_delta_topsoil_elevation_1_processed)
stat_lm_subsoil_elevation_1_main <- lm(BrayCurtisDissimilarity ~ delta_elevation, 
                                       data = df_bray_delta_subsoil_elevation_1_processed)

# Combined temperature analysis with mean of pin and deep temperatures
df_bray_delta_combined_1_processed <- df_bray_delta_topsoil_1_processed %>%
  bind_rows(df_bray_delta_subsoil_1_processed) %>%
  mutate(meanTemp = (delta_pinTemp + delta_deepTemp) / 2)

stat_lm_combined_meanTemp_1_main <- lm(BrayCurtisDissimilarity ~ meanTemp, 
                                      data = df_bray_delta_combined_1_processed %>% filter(zone1 == zone2))
...
summary(df_metadata_0_raw)

# Phyloseq objects summary
cat("\nPhyloseq raw data summary:\n")
print(ps_raw_2_filtered)

cat("\nPhyloseq rarefied data summary:\n")
print(ps_rare_2_filtered)

# Alpha diversity summary
cat("\nAlpha diversity summary:\n")
summary(df_alpha_div_3_long)

# Community dissimilarity summary
cat("\nCommunity dissimilarity summaries:\n")
cat("Topsoil Bray-Curtis dissimilarity:\n")
summary(df_bray_curtis_topsoil_2_merged$BrayCurtisDissimilarity)

cat("\nSubsoil Bray-Curtis dissimilarity:\n")
summary(df_bray_curtis_subsoil_2_merged$BrayCurtisDissimilarity)

# Environmental data summary
cat("\nTemperature delta summary:\n")
summary(df_warming_delta_1_processed)

cat("\nElevation delta summary:\n")
summary(df_elevation_delta_1_processed)

# Statistical results summary
cat("\nKey statistical results:\n")
cat("PERMANOVA main results:\n")
print(stat_permanova_bray_1_main)

cat("\nAlpha diversity ANOVA results:\n")
print(stat_anova_alpha_1_main)

cat("\nTemperature-dissimilarity relationships (R-squared values):\n")
cat("Topsoil pin temperature:", summary(stat_lm_topsoil_pin_1_main)$r.squared, "\n")
cat("Subsoil deep temperature:", summary(stat_lm_subsoil_deep_1_main)$r.squared, "\n")
cat("Combined mean temperature:", summary(stat_lm_combined_meanTemp_1_main)$r.squared, "\n")

cat("\nElevation-dissimilarity relationships (R-squared values):\n")
cat("Topsoil elevation:", summary(stat_lm_topsoil_elevation_1_main)$r.squared, "\n")
cat("Subsoil elevation:", summary(stat_lm_subsoil_elevation_1_main)$r.squared, "\n")
cat("Combined elevation:", summary(stat_lm_combined_elevation_1_main)$r.squared, "\n")
```
...
# robustlmm: Koller, M. (2024). robustlmm: Robust Linear Mixed Effects Models. R package version 3.2-2. https://CRAN.R-project.org/package=robustlmm
...
# Exportiere Mittelwert pro Plot und Layer (wie für IRIS und qPCR)
...
write.csv(df_alpha_summary_export, here("data", "rna", "df_alpha_summary.csv"), row.names = FALSE)
```
