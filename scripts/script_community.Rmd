...
```{r}
# create phyloseq objetc with raw (not rarefied data)
metadata <- metadata %>%
...
# Phyloseq objet with rarefied and filtere data (used for NMDS plots)
ps_rare = phyloseq(
...
#ps_rare$Treatment <- as.factor(ps_rare$Treatment)

# NMDS plot with Layer as horizontal facet (labels above), Zone as vertical facet (panels in columns by Layer, rows by Zone)
plot_all_treatment_layer_nmds <- ps_rare %>%
 ...
 stat_chull(ggplot2::aes(group = Treatment, fill = Treatment, colour = Treatment), geom = "polygon", alpha = 0.7) +
...
head(metadata)
metadata_1 <- metadata
row.names(metadata_1) <- metadata_1$sample_name
sam <- sample_data(metadata_1)
plaply <- phyloseq(otu_table(ps_rare, taxa_are_rows = T), sam)
pla = ordinate(ps_rare, "PCoA", "bray") 
pplant = plot_ordination(plaply, pla, type = "Zone",color = "Treatment", shape = "Treatment", axes = 1:2)+
 # scale_color_manual(values = c("#4393c3", "#F6B394", "#cd4e44"))+
  ...
  stat_ellipse(aes(color = Treatment),level=0.65,linetype=2); pplant
...
## Step 3b: PERMANOVA aitchinson (Supplement figure)
```{r}
# Check permutations with permustats
permustat(adonis2(dist_matrix ~ Zone, data = metadata))

stat_PERM_bray_int <- stat_PERM_bray %>%
  dist_permanova(
    seed = 1,
...
# Set order of Phyla to be descending and inlcuding others at the bottom
...
plot_alpha_div_zone <- ggplot(df_alpha_div_long, aes(x = Zone, y = Value, fill = Zone)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Boxplots for diversity
  geom_jitter(width = 0.2, alpha = 0.7, aes(colour = Zone), size = 2) +  # Points overlay
  ...
  theme_JM +
  labs(x = "Zone", y = "Alpha Diversity") +
  theme(strip.background = element_rect(fill = "#5ab4ac"))

plot_alpha_div_zone_treatment <- ggplot(df_alpha_div_long, aes(x = Zone, y = Value, fill = Treatment)) +
...
# Step 7: Warming data 
## Step 7a: Calculate dissimalarities
```{r}
...
plot_bray_delta_combined <- ggarrange(
  plot_bray_delta_top_elevation, 
  plot_bray_delta_top_ri,
  plot_bray_delta_top_pin, 
  plot_bray_delta_bottom_elevation, 
  plot_bray_delta_bottom_ri,
  plot_bray_delta_bottom_deep,
  ncol = 3, 
  nrow = 3,
...
# Prepare warming delta data
unique_plots <- unique(df_warming_all_2022_monthly$plotid)

# Create ALL pairwise combinations 
df_all_pairs <- expand.grid(
  plotid1 = unique_plots,
  plotid2 = unique_plots,
  stringsAsFactors = FALSE
) 

df_warming_delta_full <- df_all_pairs %>%
  left_join(
    df_warming_all_2022_monthly %>%
      select(plotid, pinTemp_1 = pinTemp_mean, deepTemp_1 = deepTemp_mean),
    by = c("plotid1" = "plotid")
  ) %>%
  left_join(
    df_warming_all_2022_monthly %>%
      select(plotid, pinTemp_2 = pinTemp_mean, deepTemp_2 = deepTemp_mean),
    by = c("plotid2" = "plotid")
  ) %>%
  mutate(
    delta_pinTemp = abs(pinTemp_1 - pinTemp_2),
    delta_deepTemp = abs(deepTemp_1 - deepTemp_2)
  ) 

# Prepare Bray-Curtis distance data  
df_bray_curtis_top_plots_mean <- df_bray_curtis_top_plots %>%
  group_by(plotid1, plotid2) %>%
  summarise(BrayCurtisDissimilarity = mean(BrayCurtisDissimilarity))

df_bray_top_complete <- df_bray_curtis_top_plots_mean %>%
  left_join(df_warming_delta_full, by = c("plotid1", "plotid2"))

# Create Bray-Curtis distance matrix (27x27)
mat_bray_top_dist <- df_bray_top_complete %>%
  group_by(plotid1, plotid2) %>%
  summarize(BrayCurtisDissimilarity = mean(BrayCurtisDissimilarity)) %>% 
  pivot_wider(names_from = plotid2, values_from = BrayCurtisDissimilarity) %>%
  column_to_rownames("plotid1") %>%
  as.matrix() %>%
  as.dist()

# Create delta temperature (pin 20 cm) mateix (27 x 27)
mat_delta_pin_dist <- df_bray_top_complete %>%
  group_by(plotid1, plotid2) %>%
  summarize(delta_pinTemp = mean(delta_pinTemp)) %>% 
  pivot_wider(names_from = plotid2, values_from = delta_pinTemp) %>%
  column_to_rownames("plotid1") %>%
  as.matrix() %>%
  as.dist()

# Run Mantel test (handles NAs if present)
mantel_result <- mantel(
  mat_bray_top_dist, 
  mat_delta_pin_dist, 
  method = "pearson", 
  permutations = 999,
  na.rm = TRUE  # Exclude NA pairs from calculation
)
...
```{r,  message=FALSE, warning=FALSE}
# Transform asv table into long-format. Transformation will catch all headers (sample names) into a new coloumn (Sample_ID) 
asv_long <- asv %>%  
...
# Plot: Bray-Curtis-Dissimilarität vs. delta_ri für Top- und Bottom-Soil
# Layer-Namen anpassen: "bottom-soil (40-80 cm)" für Bottom-Soil

df_pairs <- df_pairs %>%
  mutate(Layer = ifelse(Layer == "Bottom-Soil" | Layer == "bottom-soil", "bottom-soil (40-80 cm)",
                       ifelse(Layer == "Top-Soil" | Layer == "top-soil", "top-soil (0-40 cm)", Layer)))

p_delta_ri <- ggplot(df_pairs, aes(x = delta_ri, y = mean_bc)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  facet_wrap(~ Layer, ncol = 2, labeller = label_both) +
  labs(x = expression(Delta~Soil~Reduction~(Delta~r[i])),
       y = "Bray-Curtis Dissimilarity",
       title = "Bray-Curtis Dissimilarity vs. Delta Soil Reduction (ri)") +
