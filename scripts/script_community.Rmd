...
# Reorder Factors
ps@sam_data[["Zone"]] <- dplyr::recode(ps@sam_data[["Zone"]],
  "PZ" = "Pioneer zone",
  "LM" = "Low marsh",
  "HM" = "High marsh"
)
ps@sam_data[["Zone"]] <- factor(ps@sam_data[["Zone"]], levels = c("Pioneer zone","Low marsh","High marsh"))
...
ps@sam_data[["Layer"]]  <- factor(ps@sam_data[["Layer"]],levels=c("Topsoil (0-30 cm)","Subsoil (40-100 cm)"))
...
# Reorder Factors
ps_rare@sam_data[["Zone"]] <- dplyr::recode(ps_rare@sam_data[["Zone"]],
  "PZ" = "Pioneer zone",
  "LM" = "Low marsh",
  "HM" = "High marsh"
)
ps_rare@sam_data[["Zone"]] <- factor(ps_rare@sam_data[["Zone"]], levels = c("Pioneer zone","Low marsh","High marsh"))
...
ps_rare@sam_data[["Layer"]]  <- factor(ps_rare@sam_data[["Layer"]],levels=c("Topsoil (0-30 cm)","Subsoil (40-100 cm)"))
...
# Summarise for topsoil samples
df_abundance_data_top <- ps_phylum_rel %>%
  psmelt() %>%
  mutate(Phylum = tax_table(ps_phylum_rel)[OTU, "Phylum"]) %>%
  mutate(Phylum = ifelse(Phylum %in% top_phyla, as.character(Phylum), "Others")) %>%
...
# Summarise for subsoil samples
df_abundance_data_sub <- ps_phylum_rel %>%
  psmelt() %>%
  mutate(Phylum = tax_table(ps_phylum_rel)[OTU, "Phylum"]) %>%
  mutate(Phylum = ifelse(Phylum %in% top_phyla, as.character(Phylum), "Others")) %>%
...
# See share of groups
df_temporal <- df_abundance_data_layer %>%
  group_by(Phylum) %>%
...
# Set factor levels for topsoil data
df_abundance_data_top$Phylum <- factor(df_abundance_data_top$Phylum, 
                                      levels = c("Gemmatimonadota", "Myxococcota", "Actinobacteriota", 
                                               "Planctomycetota", "Acidobacteriota", "Desulfobacterota", 
                                               "Firmicutes", "Chloroflexi", "Proteobacteria", "Others"))

# Set factor levels for subsoil data  
df_abundance_data_sub$Phylum <- factor(df_abundance_data_sub$Phylum, 
                                      levels = c("Gemmatimonadota", "Myxococcota", "Actinobacteriota", 
                                               "Planctomycetota", "Acidobacteriota", "Desulfobacterota", 
                                               "Firmicutes", "Chloroflexi", "Proteobacteria", "Others"))
...
plot_stacked_bar_top <- ggplot(df_abundance_data_top, aes(x = Treatment, y = Abundance, fill = Phylum)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = c("#D73027", "#FDAE61", "#ffff99", "#b2df8a", "#33a02c", "#cab2d6", "#1f78b4", "#a6cee3", "black", "grey"))+
  theme_JM +
  labs(y = "Relative Abundance [%]") +
...
plot_stacked_bar_sub <- ggplot(df_abundance_data_sub, aes(x = Treatment, y = Abundance, fill = Phylum)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = c("#D73027", "#FDAE61", "#ffff99", "#b2df8a", "#33a02c", "#cab2d6", "#1f78b4", "#a6cee3", "black", "grey"))+
  theme_JM +
  labs(y = "Relative Abundance [%]") +
...
# Final plot
plot_barplot_final <- ggarrange(plot_stacked_bar_top, plot_stacked_bar_sub,
                               labels = c("A", "B"), 
                               ncol = 1,
                               common.legend = TRUE,
                               legend = "top")
...
plot_barplot_final <- annotate_figure(plot_barplot_final,
                                     right = textGrob(c("Topsoil (0-30 cm)", "Subsoil (40-100 cm)"), 
                                                     x = c(0.5, 0.5),
                                                     y = c(0.7, 0.25),  
                                                     rot = 270,
...
df_alpha_div_long$Zone <- dplyr::recode(df_alpha_div_long$Zone,
  "PZ" = "Pioneer zone",
  "LM" = "Low marsh",
  "HM" = "High marsh"
)
df_alpha_div_long$Zone <- factor(df_alpha_div_long$Zone, levels = c("Pioneer zone", "Low marsh", "High marsh"))
df_alpha_div_long$Treatment <- recode_factor(df_alpha_div_long$Treatment, "ambient" = "ambient", "1.5" = "+1.5°C", "3" = "+3.0°C")
df_alpha_div_long$Layer <- factor(df_alpha_div_long$Layer, levels = c("Topsoil\n(0-30 cm)", "Subsoil\n(40-100 cm)"))

# Plot mit neuen Levels
...
shapiro.test(resid(stat_model_alpha))

# Log-transformation and model if not normal
stat_model_alpha_log <- lmer(log(Value + 1) ~ Treatment * Zone * Layer + (1|Plot), data = df_alpha_div_long)
shapiro.test(resid(stat_model_alpha_log))

# Robust mixed model
if (!requireNamespace("robustlmm", quietly = TRUE)) install.packages("robustlmm")
library(robustlmm)
stat_model_alpha_robust <- rlmer(Value ~ Treatment * Zone * Layer + (1|Plot), data = df_alpha_div_long)
summary(stat_model_alpha_robust)

# Pairwise comparisons for robust model
library(emmeans)
# Main effects
stat_alpha_zone_emm_robust <- emmeans(stat_model_alpha_robust, ~ Zone)
pairs(stat_alpha_zone_emm_robust)
stat_alpha_treatment_emm_robust <- emmeans(stat_model_alpha_robust, ~ Treatment)
pairs(stat_alpha_treatment_emm_robust)
stat_alpha_layer_emm_robust <- emmeans(stat_model_alpha_robust, ~ Layer)
pairs(stat_alpha_layer_emm_robust)
# Interactions
stat_alpha_zone_treatment_emm_robust <- emmeans(stat_model_alpha_robust, ~ Zone | Treatment)
pairs(stat_alpha_zone_treatment_emm_robust)
stat_alpha_zone_layer_emm_robust <- emmeans(stat_model_alpha_robust, ~ Zone | Layer)
pairs(stat_alpha_zone_layer_emm_robust)
stat_alpha_treatment_layer_emm_robust <- emmeans(stat_model_alpha_robust, ~ Treatment | Layer)
pairs(stat_alpha_treatment_layer_emm_robust)
stat_alpha_treatment_layer_zone_emm_robust <- emmeans(stat_model_alpha_robust, ~ Treatment | Layer | Zone)
pairs(stat_alpha_treatment_layer_zone_emm_robust)
```
...
```{r}
# Filter the phyloseq object for Topsoil
ps_top <- subset_samples(ps_rare, Layer == "Topsoil (0-30 cm)")
# Filter the phyloseq object for Subsoil
ps_sub <- subset_samples(ps_rare, Layer == "Subsoil (40-100 cm)")

# Calculate Bray-Curtis distances on the filtered object
stat_bray_curtis_dist_top <- phyloseq::distance(ps_top, method = "bray")
stat_bray_curtis_dist_bottom <- phyloseq::distance(ps_sub, method = "bray")
...
#stat_bray_curtis_dist_bottom <- phyloseq::distance(ps_sub, method = "euclidean", transform = "clr")
...
if (exists("df_bray_curtis_top_plots")) {
  df_bray_delta_top_ri <- df_bray_curtis_top_plots %>%
    left_join(df_ri_delta %>% filter(Layer == "Topsoil (0-30 cm)"), by = c("plotid1", "plotid2")) %>%
    left_join(df_ri_delta %>% filter(Layer == "Topsoil (0-30 cm)"), by = c("plotid2" = "plotid1", "plotid1" = "plotid2"), suffix = c("", "_rev")) %>%
...
if (exists("df_bray_curtis_bottom_plots")) {
  df_bray_delta_bottom_ri <- df_bray_curtis_bottom_plots %>%
    left_join(df_ri_delta %>% filter(Layer == "Subsoil (40-80 cm)"), by = c("plotid1", "plotid2")) %>%
    left_join(df_ri_delta %>% filter(Layer == "Subsoil (40-80 cm)"), by = c("plotid2" = "plotid1", "plotid1" = "plotid2"), suffix = c("", "_rev")) %>%
    mutate(delta_ri = coalesce(delta_ri, delta_ri_rev)) %>%
    select(sample_name, comparison, plotid1, plotid2, BrayCurtisDissimilarity, delta_ri, everything(), -ends_with("_rev")) %>%
    rowwise() %>%
    mutate(sorted_plotids = paste(sort(c(plotid1, plotid2)), collapse = "_")) %>%
...
# DT: Xie, Y. (2024). DT: A Wrapper of the JavaScript Library 'DataTables'. R package version 0.33. https://CRAN.R-project.org/package=DT
...
# ggside: Lin, H. (2024). ggside: Side Grammar Graphics for 'ggplot2'. R package version 0.3.1. https://CRAN.R-project.org/package=ggside
...
# knitr: Xie, Y. (2024). knitr: A General-Purpose Package for Dynamic Report Generation in R. R package version 1.45. https://CRAN.R-project.org/package=knitr
