...
```{r}
df_functional <- read.delim("./data/faprotax/df_functional_database.tsv", dec = ".", sep = "\t")
...
## Step 2b: Barplots
```{r}
plot_degradation_bar <- df_merged_filtered %>%
  filter(Functional %in% desired_functions_degradation) %>%
  ggplot(aes(x = abundance, y = Functional, fill = Treatment)) +
