...
if(!is.null(df_warming_1_monthly)) {
  ...
  temp_cols <- df_eea_5_summary %>% dplyr::select(contains("Temp_mean")) %>% names()
  
  if(length(temp_cols) > 0) {
    df_eea_5_summary <- df_eea_5_summary %>%
      mutate("Soil temperature" = rowMeans(dplyr::select(., all_of(temp_cols)), na.rm = TRUE))
  } else {
...
if(!is.null(df_warming_1_monthly) && "Elevation_mean" %in% colnames(df_eea_4_final)) {
  ...
  if(all(c("pinTemp_mean", "deepTemp_mean") %in% colnames(df_eea_4_final))) {
    df_eea_4_final <- df_eea_4_final %>%
      mutate(meanTemp = rowMeans(dplyr::select(., pinTemp_mean, deepTemp_mean), na.rm = TRUE))
...
tiff(here("output", "plot_CN_deviations.tiff"), 
     width = 12, height = 10, units = "in", res = 600, compression = "lzw")
print(plot_CN_deviations)
dev.off()

# Also save as EPS for vector graphics option
postscript(here("output", "plot_CN_deviations.eps"), 
