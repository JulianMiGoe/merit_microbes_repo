...
# Plot: Sample size per plotid (after filtering)
df_plotid_counts <- df_EEA_merged %>%
  group_by(plotid = substring(ID.y, 1, 3), Enzyme) %>%
  summarise(n = n())
...
ggplot(df_plotid_counts, aes(x = plotid, y = n, fill = as.factor(n))) +
  ...
  geom_text(aes(label = n), vjust = -0.5, size = 4) +
  scale_fill_brewer(palette = "Set2", name = "Sample size") +
  labs(x = "Plot ID", y = "Number of samples", title = "Sample size per plotid after filtering") +
  theme_minimal() +
  facet_wrap(~Enzyme) +
  theme(legend.position = "none")

# Summarise technical replicates by Blank, Standard, Sub_ctrl, Hom_ctrl, Quench, Assay
# Alle anderen Spalten, die nicht aggregiert werden, werden mit first() übernommen
df_EEA_merged <- df_EEA_merged_filtered %>%
  group_by(Blank, Standard, Sub_ctrl, Hom_ctrl, Quench, Assay) %>%
  summarise(
    n = n(),
    ...
    UID = first(UID),
    Depth.x = first(Depth.x),
    Zone = first(Zone),
    Treatment = first(Treatment),
    Enzyme = first(Enzyme),
    ID.y = first(ID.y),
    Layer = first(Layer),
    # ggf. weitere Spalten ergänzen, falls benötigt
    .groups = "drop"
  )

ggplot(df_EEA_merged) +
  aes(x = Activity_OM_mean) +
  geom_histogram(
    bins = round(sqrt(length(df_EEA_merged$Activity_OM_mean))),
    fill = "steelblue", color = "black"
  ) +
  theme_minimal()
