...
# Versionlog
## Version 1.0: basic function to load data from raw files (08.06.2024)
## Version 1.1: EEA data processing routine (11.02.2025)
...
file_numbers <- sprintf("%03d", c(1:100, 102:325))
...
df_EEA_merged <- df_EEA_merged %>%
  mutate(
    Enzyme = case_when(
      ...
      Plate.x %in% c(paste0("Plate_", sprintf("%03d", 110:214)), 
                     "Plate_324", 
                     "Plate_325") ~ "CHI",  # Explicitly add 324/325
      Plate.x %in% paste0("Plate_", sprintf("%03d", 215:323)) ~ "LEU",
      TRUE ~ NA_character_
    )
  )
...
df_EEA_merged <- df_EEA_merged %>%
  mutate(Inc_start        = as.POSIXct(df_EEA_merged[,25], format="%H:%M"))%>%
  mutate(Inc_end          = as.POSIXct(df_EEA_merged[,26], format="%H:%M"))%>%
...
# Filter of Assays where Blanks were contaminated
hist(df_EEA_merged$Blank)
df_EEA_merged$z_Blank <- scale(df_EEA_merged$Blank)
hist(df_EEA_merged$z_Blank)
summary(df_EEA_merged$z_Blank)
which(df_EEA_merged$z_Blank > 3.29)
qqPlot(df_EEA_merged$Blank)
mean(df_EEA_merged$Blank) + 2*sd(df_EEA_merged$Blank) # = 5049.276

# Filter for Assays where substrate control was significantly upregulated, hinting at pipetting errors or contaminated buffer
hist(df_EEA_merged$Sub_ctrl)
mean(df_EEA_merged$Sub_ctrl) + 2*sd(df_EEA_merged$Sub_ctrl) # = 773726.2 

df_EEA_merged_filtered <- df_EEA_merged %>%
  filter(!Blank >= 5049.276) %>% # True in 9 cases 
  filter(!Sub_ctrl >= 773726.2) %>% # True in  87cases
  filter(!Quench_coeff <= 0.1) %>% # True in 144 cases
  filter(!Activity_OM <= 0) 
...
# calculate the mean acitivity from the four technical replicates of each plate. The highest and the lowest of the four values are removed.
df_EEA_merged <- df_EEA_merged_filtered %>%
  group_by(Depth.x, Zone, Treatment, Enzyme, UID, ID.y, Layer) %>%
  mutate(rank = rank(Activity_OM, ties.method = "first")) %>%
  filter(rank %in% c(2, 3)) %>%
  summarise(n = n(),
            Activity_OM_mean = mean(Activity_OM, na.rm = TRUE),
            sd = sd(Activity_OM, na.rm = TRUE),
            se = sd / sqrt(n),
            ci = qt(0.975, df = n - 1) * sd / sqrt(n),
            .groups = "drop")

df_EEA_merged$Activity_OM <- df_EEA_merged$Activity_OM_mean
...
df_glu_data <- df_EEA_merged %>%
  #filter(!Activity_FW <= 0 & !Activity_FW >= 1000)%>%  
  filter(Enzyme == "GLU") %>%
...
df_EEA_sum <- df_EEA_merged %>%
  ...
  summarise(Activity_OM_mean = mean(Activity_OM_mean, na.rm = TRUE))
...
#df_EEA_merged$Layer <- df_EEA_merged$Layer.x
```


# Step 4: Plots
## Step 4a: EEAs (factorial)
```{r}
# Plot C-Acquisition
plot_EEA_mean_layer_GLU <- df_EEA_merged %>%
  ...
  ggplot(aes(x = Treatment, y = Activity_OM, fill = Treatment)) +
    ...
    theme(strip.background = element_rect(fill = "grey44"),
...
plot_EEA_C_warming_pin <- df_EEA_final %>%
  ...
  ggplot(aes(x = pinTemp_mean, y = Activity_C, color = Zone)) +
    ...
    theme_JM + 
    labs(x = "Monthly T at 20 depth [°C]") 
...
## Step 4c: EEAs (correaltion)
```{r}
library(ggplot2)
library(ggpubr)

# ---- Top-soil, C-acquisition ----
plot_EEA_C_warming_pin <- df_EEA_final %>%
  filter(grepl("Top-soil", Layer)) %>%
  ggplot(aes(x = pinTemp_mean, y = Activity_C, color = Zone)) +
    geom_point() +
...
# ---- Arrange all plots ----
plot_CN_warming_pin_deep_cor <- ggarrange(
  plot_EEA_C_warming_pin,
  plot_EEA_N_warming_pin,
  plot_EEA_C_warming_deep,
  plot_EEA_N_warming_deep,
...
plot_CN_warming_pin_deep_cor
```


## Step 4d: Plots EEA-ratio
```{r}
...
## Step 4d: Plots Redox, Elevation, EEA
```{r}
# Group Enzymes by C and N Acquistion
df_EEA_merged <- df_EEA_merged %>%
  mutate(
    Enzyme_group = case_when(
      Enzyme == "GLU" ~ "C-Acquisition", # GLU -> C-Acquisition
      Enzyme %in% c("CHI", "LEU") ~ "N-Acquisition", # CHI or LEU -> N-Acquisition
      TRUE ~ NA_character_ # Default: NA for other values
    )
  )

plot_reg_ri_eea <- df_EEA_merged %>%
  filter(!is.na(ri) & !is.na(Activity_OM_mean) & !is.na(Enzyme_group)) %>% # Remove rows with NA
  ggplot(aes(x = ri, y = Activity_OM_mean, color = Enzyme_group)) +
  geom_point() +
  ...
  facet_wrap(~ Layer, ncol = 1) + 
     ...
     scale_y_continuous(name = expression(paste("Enzymatic activity", " ", " [", "nmol", "*", "g", " ", "OM"^-1, "*", "h"^-1, "]")), expand = c(0,0)) +
  labs(
    x = "Reduction index") +
  scale_color_manual(
    values = c("C-Acquisition" = "#d8b365", "N-Acquisition" = "#5ab4ac"),
    name = "Enzymes:"
  ) +
  theme_JM

ggarrange(plot_reg_ri_eea, plot_iris_bar)

# add plot ID for merging
df_EEA_merged$Plot <- substring(df_EEA_merged$ID.y, 1,3)

# summarise EEA dataframe for Glucosidase activity
df_EEA_sum  <- df_EEA_merged  %>%
  filter(Enzyme == "GLU")%>%
  group_by(Zone, Treatment, Enzyme, Plot) %>%
  summarize(n = n(),
            mean_activity = mean(Activity_OM_mean, na.rm=TRUE),
            sd = sd(Activity_OM_mean, na.rm=TRUE),
            se = sd / sqrt(n),
            ci = qt(0.975, df = n - 1) * sd / sqrt(n))

# summarise iris (ri) and elevation dataframe 
df_iris_elevation_sum  <- df_elevation  %>%
  group_by(Treatment, Plot) %>%
  summarize(n = n(),
            mean_ri = mean(mean_ri, na.rm=TRUE),
            sd = sd(mean_ri,na.rm=TRUE),
            se = sd / sqrt(n),
            ci = qt(0.975, df = n - 1) * sd / sqrt(n),
            mean_elevation= mean(elevation, na.rm=TRUE))

df_iris_elevation_sum$Plot <- as.factor(df_iris_elevation_sum$Plot)

# merge dataframe EEA & iris/elevation
df_eea_elevation <- df_EEA_sum  %>%
   left_join(df_iris_elevation_sum, by = c("Plot"))
 
 
 my_colors <- c("darkblue","#4575B4", "yellow")
 my_gradient <- c("#fee8c8","#fdbb84","#e34a33","#7f0000")
 
plot_iris_elevation_eea <-ggplot(df_EEA_final, aes(x = ri  , y = Elevation_mean, color= Activity_N)) +
  geom_point(aes(x = ri  , y = Elevation_mean, color = Activity_C), size=3) +
  geom_smooth(method = "loess", se = TRUE)+
  ylab(expression("Elevation in relation to MHT [cm]"))+
  xlab(expression("Reduction Index"))+
  scale_color_gradientn(colors = my_gradient, na.value = "white")  +
  theme_JM+
  coord_flip()

# Plot incl depth (mathced were possible)
# summarise EEA dataframe for Glucosidase activity
df_EEA_depth_sum  <- df_EEA_merged  %>%
  filter(Enzyme == "GLU")%>%
  group_by(Zone, Treatment, Enzyme, Plot, Depth.x) %>%
  summarize(n = n(),
            mean_activity = mean(Activity_FW, na.rm=TRUE),
            sd = sd(Activity_FW,na.rm=TRUE),
            se = sd / sqrt(n),
            ci = qt(0.975, df = n - 1) * sd / sqrt(n))

df_EEA_depth_sum$Depth <- df_EEA_depth_sum$Depth.x

# summarise reduction index from 60-70 cm and 70-80 cm to 60-80 cm, to match the EEA data
df_iris_elevation <- df_iris_elevation %>%
mutate(Depth = ifelse(Depth %in% c("60-70 cm", "70-80 cm"), "60-80 cm", Depth))%>%
mutate(Depth = ifelse(Depth %in% c("0-10 cm"), "5-10 cm", Depth))

# summarise iris (ri) and elevation dataframe 
df_iris_elevation_depth_sum  <- df_iris_elevation  %>%
  group_by(Zone, Treatment, Plot, Depth) %>%
  summarize(n = n(),
            mean_ri = mean(mean_ri, na.rm=TRUE),
            sd = sd(mean_ri,na.rm=TRUE),
            se = sd / sqrt(n),
...
df_iris_elevation_depth_sum$Plot  <- as.factor(df_iris_elevation_depth_sum$Plot)

# merge dataframe EEA & iris/elevation
df_iris_elevation_depth_EEA <- df_EEA_depth_sum  %>%
   left_join(df_iris_elevation_depth_sum, by = c("Plot","Depth")) %>%
    filter(!is.na(mean_ri)) 

plot_iris_elevation_depth_eea <-ggplot(df_iris_elevation_depth_EEA, aes(x = mean_ri  , y = mean_elevation, color=mean_activity)) +
  geom_point(aes(x = mean_ri  , y = mean_elevation, color=mean_activity), size=3) +
  geom_smooth(method = "loess", se = TRUE, color="#2b8cbe",fill="#a6bddb")+
  ylab(expression("Elevation in relation to MHT [m]"))+
  xlab(expression("Reduction Index"))+
  scale_color_gradientn(colors = my_gradient, na.value = "white",  name = expression(atop("ß-Glucosidase activity", paste("[", "nmol", "*", "g", " ", DW^-1, "*", "h"^-1, "]"))))+
  #scale_x_reverse()+
  theme_JM+
  coord_flip()

plot_iris_elevation_eea
plot_iris_elevation_depth_eea
```
