...
## Version 2.0: EEA analysis routine - streamlined and cleaned - 11.02.2025 | JM
...
# Log transformation and re-test
df_eea_1_main_log <- df_eea_1_main %>%
  mutate(Activity_OM_log = log(Activity_OM + 1))

print("=== NORMALITY TESTS AFTER LOG TRANSFORMATION ===")

# Test log-transformed data
eea_c_log_data <- df_eea_1_main_log %>% filter(Enzyme == "GLU", !is.na(Activity_OM_log))
if(nrow(eea_c_log_data) <= 5000) {
  shapiro_c_log <- shapiro.test(eea_c_log_data$Activity_OM_log)
  print("Log-transformed carbon acquisition normality test:")
  print(shapiro_c_log)
}

eea_n_log_data <- df_eea_1_main_log %>% filter(Enzyme %in% c("CHI", "LEU"), !is.na(Activity_OM_log))
if(nrow(eea_n_log_data) <= 5000) {
  shapiro_n_log <- shapiro.test(eea_n_log_data$Activity_OM_log)
  print("Log-transformed nitrogen acquisition normality test:")
  print(shapiro_n_log)
}
```
...
## Step 5c: Log-transformed models
```{r}
# Log-transformed models
print("=== LOG-TRANSFORMED MIXED-EFFECTS MODELS ===")

stat_model_C_log <- lmer(log(Activity_OM + 1) ~ Zone * Treatment * Layer + (1|Plot/core_id), 
                         data = df_eea_1_main %>% filter(Enzyme == "GLU"))

stat_model_N_log <- lmer(log(Activity_OM + 1) ~ Zone * Treatment * Layer + (1|Plot/core_id), 
                         data = df_eea_1_main %>% filter(Enzyme %in% c("LEU", "CHI")))

# Test residual normality for log models
print("Residual normality tests for log-transformed models:")
shapiro.test(resid(stat_model_C_log))
shapiro.test(resid(stat_model_N_log))

# ANOVA results for log models
print("=== ANOVA RESULTS - LOG-TRANSFORMED MODELS ===")
print("Log-transformed carbon acquisition:")
anova(stat_model_C_log)

print("Log-transformed nitrogen acquisition:")
anova(stat_model_N_log)
```

## Step 5d: Robust mixed-effects models
```{r}
# Robust linear mixed-effects models
print("=== ROBUST LINEAR MIXED-EFFECTS MODELS ===")

stat_model_C_robust <- rlmer(Activity_OM ~ Zone * Treatment * Layer + (1|Plot/core_id), 
                             data = df_eea_1_main %>% filter(Enzyme == "GLU"))

stat_model_N_robust <- rlmer(Activity_OM ~ Zone * Treatment * Layer + (1|Plot/core_id), 
                             data = df_eea_1_main %>% filter(Enzyme %in% c("LEU", "CHI")))

# Model summaries
print("Robust carbon acquisition model:")
summary(stat_model_C_robust)

print("Robust nitrogen acquisition model:")
summary(stat_model_N_robust)

# Note: Use summary() instead of anova() for robust models
print("Use summary() results above for robust model interpretation")
```

## Step 5e: Pairwise comparisons using emmeans
```{r}
# Choose the best performing model (robust models recommended)
print("=== PAIRWISE COMPARISONS - ROBUST MODELS ===")

# Main effects
print("--- ZONE COMPARISONS ---")
stat_C_zone_emm_robust <- emmeans(stat_model_C_robust, ~ Zone)
print("Carbon acquisition by Zone:")
pairs(stat_C_zone_emm_robust)

stat_N_zone_emm_robust <- emmeans(stat_model_N_robust, ~ Zone)
print("Nitrogen acquisition by Zone:")
pairs(stat_N_zone_emm_robust)

print("--- TREATMENT COMPARISONS ---")
stat_C_treatment_emm_robust <- emmeans(stat_model_C_robust, ~ Treatment)
print("Carbon acquisition by Treatment:")
pairs(stat_C_treatment_emm_robust)

stat_N_treatment_emm_robust <- emmeans(stat_model_N_robust, ~ Treatment)
print("Nitrogen acquisition by Treatment:")
pairs(stat_N_treatment_emm_robust)

print("--- LAYER COMPARISONS ---")
stat_C_layer_emm_robust <- emmeans(stat_model_C_robust, ~ Layer)
print("Carbon acquisition by Layer:")
pairs(stat_C_layer_emm_robust)

stat_N_layer_emm_robust <- emmeans(stat_model_N_robust, ~ Layer)
print("Nitrogen acquisition by Layer:")
pairs(stat_N_layer_emm_robust)

# Interaction effects
print("--- ZONE x TREATMENT INTERACTIONS ---")
stat_C_zone_treatment_emm_robust <- emmeans(stat_model_C_robust, ~ Zone | Treatment)
print("Carbon acquisition Zone x Treatment:")
pairs(stat_C_zone_treatment_emm_robust)

stat_N_zone_treatment_emm_robust <- emmeans(stat_model_N_robust, ~ Zone | Treatment)
print("Nitrogen acquisition Zone x Treatment:")
pairs(stat_N_zone_treatment_emm_robust)

print("--- ZONE x LAYER INTERACTIONS ---")
stat_C_zone_layer_emm_robust <- emmeans(stat_model_C_robust, ~ Zone | Layer)
print("Carbon acquisition Zone x Layer:")
pairs(stat_C_zone_layer_emm_robust)

stat_N_zone_layer_emm_robust <- emmeans(stat_model_N_robust, ~ Zone | Layer)
print("Nitrogen acquisition Zone x Layer:")
pairs(stat_N_zone_layer_emm_robust)

print("--- TREATMENT x LAYER INTERACTIONS ---")
stat_C_treatment_layer_emm_robust <- emmeans(stat_model_C_robust, ~ Treatment | Layer)
print("Carbon acquisition Treatment x Layer:")
pairs(stat_C_treatment_layer_emm_robust)

stat_N_treatment_layer_emm_robust <- emmeans(stat_model_N_robust, ~ Treatment | Layer)
print("Nitrogen acquisition Treatment x Layer:")
pairs(stat_N_treatment_layer_emm_robust)

print("--- THREE-WAY INTERACTIONS ---")
stat_C_treatment_layer_zone_emm_robust <- emmeans(stat_model_C_robust, ~ Treatment | Layer | Zone)
print("Carbon acquisition Treatment x Layer x Zone:")
pairs(stat_C_treatment_layer_zone_emm_robust)

stat_N_treatment_layer_zone_emm_robust <- emmeans(stat_model_N_robust, ~ Treatment | Layer | Zone)
print("Nitrogen acquisition Treatment x Layer x Zone:")
pairs(stat_N_treatment_layer_zone_emm_robust)
```

## Step 5f: Regression analyses with environmental variables
```{r}
...
# robustlmm: Koller, M. (2024). robustlmm: Robust Linear Mixed Effects Models. R package version 3.3-1. https://CRAN.R-project.org/package=robustlmm
