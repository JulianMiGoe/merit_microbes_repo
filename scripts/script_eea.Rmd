...
df_EEA_merged$Zone <- dplyr::recode(df_EEA_merged$Zone,
  "PZ" = "Pioneer zone",
  "LM" = "Low marsh",
  "HM" = "High marsh"
)
df_EEA_merged$Zone <- factor(df_EEA_merged$Zone, levels = c("Pioneer zone", "Low marsh", "High marsh"),
...
df_EEA_merged$Layer <- factor(df_EEA_merged$Layer, levels = c("Topsoil\n(0-30 cm)", "Subsoil\n(40-100 cm)"))
...
```{r}
# Regression-Plot: Anpassung der Farben und Legenden
...
plot_EEA_N_warming_pin <- df_EEA_final %>%
 # filter(Enzyme %in% c("CHI", "LEU")) %>%
  ...
  ggplot(aes(x = pinTemp_mean, y = Activity_N, color = Zone)) +
    ...
    scale_color_manual(
      values = c("Pioneer zone" = "#084081", "Low marsh" = "#4eb3d3", "High marsh" = "#7fcdbb"),
      name = "Zone:"
    ) +
    scale_y_continuous(
      name = "Nitrogen acquisition\n[nmol g OM⁻¹ h⁻¹]",
      expand = c(0, 0),
      limits = c(0, 3500),
      breaks = seq(0, 3000, by = 1000)
    ) +
...
# Log-Transformation der abhängigen Variable und erneute Modellierung
stat_model_C_log <- lmer(log(Activity_OM + 1) ~ Zone * Treatment * Layer + (1|Plot/core_id), data = df_EEA_merged %>% filter(Enzyme == "GLU"))
stat_model_N_log <- lmer(log(Activity_OM + 1) ~ Zone * Treatment * Layer + (1|Plot/core_id), data = df_EEA_merged %>% filter(Enzyme %in% c("LEU", "CHI")))

# Shapiro-Wilk-Test für die Residuen der log-transformierten Modelle
shapiro.test(resid(stat_model_C_log))
shapiro.test(resid(stat_model_N_log))

# QQ-Plot für die log-transformierten Modelle
qqnorm(resid(stat_model_C_log))
qqline(resid(stat_model_C_log))
qqnorm(resid(stat_model_N_log))
qqline(resid(stat_model_N_log))

# Robustes gemischtes Modell mit robustlmm::rlmer
if (!requireNamespace("robustlmm", quietly = TRUE)) install.packages("robustlmm")
library(robustlmm)

stat_model_C_robust <- rlmer(Activity_OM ~ Zone * Treatment * Layer + (1|Plot/core_id), data = df_EEA_merged %>% filter(Enzyme == "GLU"))
stat_model_N_robust <- rlmer(Activity_OM ~ Zone * Treatment * Layer + (1|Plot/core_id), data = df_EEA_merged %>% filter(Enzyme %in% c("LEU", "CHI")))

summary(stat_model_C_robust)
summary(stat_model_N_robust)

# Main effects für robuste Modelle
anova(stat_model_C_robust)
anova(stat_model_N_robust)

# Pairwise Comparisons für robuste Modelle
library(emmeans)
# Zone
...
df_summary_all <- df_summary_all %>%
  rename(
    "Nitrogen acquisition" = `EEA N`,
    "Carbon Acquisition" = `EEA C`,
    "Microbial abundance" = Abundance,
    "Marsh surface elevation" = `Soil elevation`,
    "Reduction index" = `Soil reduction`
  )
...
# Hmisc: Harrell, F. E. Jr, et al. (2024). Hmisc: Harrell Miscellaneous. R package version 5.1-2. https://CRAN.R-project.org/package=Hmisc
