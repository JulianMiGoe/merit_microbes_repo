...
## Step 6a: Bacterial abundance summary plot
```{r}
# Create summary plot with means and standard errors matching the provided image
plot_qpcr_summary <- df_eub_2_summary %>%
  ggplot(aes(x = Treatment, y = mean_abundance, fill = Treatment)) +
    ...
    coord_cartesian(ylim = c(9.0, 9.9)) +
...
tiff(here("output", "plot_qpcr_summary.tiff"), 
     width = 12, height = 6, units = "in", res = 600, compression = "lzw")
print(plot_qpcr_summary)
dev.off()

# Also save as EPS for vector graphics option
postscript(here("output", "plot_qpcr_summary.eps"), 
           width = 12, height = 6, horizontal = FALSE)
print(plot_qpcr_summary)
dev.off()

cat("High-resolution figures saved:\n")
cat("- plot_qpcr_summary.tiff (600 DPI)\n")
cat("- plot_qpcr_summary.eps (vector)\n")

print("=== QPCR VISUALIZATION COMPLETED ===")
print("Bacterial abundance plot created matching the provided design")
```
...
*Last modified: September 30, 2025 | JM*
