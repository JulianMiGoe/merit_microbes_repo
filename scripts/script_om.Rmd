...
df_OM_24 <- read.delim(here("data", "om", "df_OM_MERIT_2024.txt"), dec=".", sep="\t")    # L0 raw data MERIT OM 2024 @JulianMiGoe, @annaschrdr
```
...
df_OM_24 <- df_OM_24 %>% 
  select(Year, Zone, Treatment, Depth, OM, Conductivity, pH) %>%
  mutate_at(c(2:4), as.factor)
...
stat_model_OM <- lmer(OM ~ Zone * Treatment * Year + (1|Depth), data = df_OM_merged %>% filter(Depth %in% c("5", "10", "15", "20", "25")) %>% filter(!Year == "2024"))
