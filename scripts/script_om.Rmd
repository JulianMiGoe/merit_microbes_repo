...
df_om_1_processed_2019 <- df_om_0_raw_2019 %>% 
  dplyr::select(Zone, Treatment, Depth, OM) %>% 
  mutate(
    Year = 2019,
    Zone = as.factor(Zone),
...
df_om_1_processed_2021 <- df_om_0_raw_2021 %>% 
  dplyr::select(Year, Zone, Treatment, Depth, OM) %>% 
  mutate(
    Zone = as.factor(Zone),
    Treatment = as.factor(Treatment),
    Depth = as.factor(Depth),
...
tiff(here("output", "plot_om_yearly.tiff"), 
     width = 12, height = 8, units = "in", res = 600, compression = "lzw")
print(plot_om_yearly)
dev.off()

# Also save as EPS for vector graphics option
postscript(here("output", "plot_om_yearly.eps"), 
           width = 12, height = 8, horizontal = FALSE)
print(plot_om_yearly)
dev.off()

cat("High-resolution figures saved:\n")
cat("- plot_om_yearly.tiff (600 DPI)\n")
cat("- plot_om_yearly.eps (vector)\n")
```
