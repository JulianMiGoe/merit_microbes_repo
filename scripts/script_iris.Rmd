...
df_iris$Zone <- factor(df_iris$Zone, levels = c("PZ", "LM", "HM"))
# Add a new column 'layer' based on the 'Depth' column
df_iris <- df_iris %>%
  mutate(Layer = case_when(
    Depth %in% c("0-10 cm", "10-20 cm", "20-30 cm") ~ "Top-soil (0-30 cm)",
    Depth %in% c("40-50 cm", "50-60 cm", "60-70 cm", "70-80 cm") ~ "Bottom-soil (40-80 cm)",
    TRUE ~ NA_character_  # For unexpected values
  ))
df_iris$Layer <- factor(df_iris$Layer, levels = c("Top-soil (0-30 cm)", "Bottom-soil (40-80 cm)"))
```
...
```{r}
df_iris_sum <- df_iris %>%
    mutate(Layer = recode_factor(Layer, "Bottom-soil (40-80 cm)" = "Bottom-soil\n(40-100 cm)")) %>%
    mutate(Layer = recode_factor(Layer, "Top-soil (0-30 cm)" = "Top-soil\n(0-30 cm)")) %>%
  filter(!Layer == "Top-soil (0-30 cm)" | !Layer == "Bottom-soil (40-100 cm)") %>%
