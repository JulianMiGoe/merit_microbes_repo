...
df_precipitation_1_processed <- df_precipitation_0_raw %>%
  dplyr::select(Year = Jahr, Month = Monat, Precipitation = Schleswig.Holstein) %>%
  mutate(
...
df_temperature_1_processed <- df_temperature_0_raw %>%
  dplyr::select(Year = Jahr, Month = Monat, Temperature = Schleswig.Holstein) %>%
  mutate(
    Year = as.numeric(Year),
    Month = as.numeric(Month),
...
# Export high-resolution TIFF file for publication (400 DPI minimum)
tiff(here("output", "plot_climate_overview.tiff"), 
     width = 10, height = 6, units = "in", res = 600, compression = "lzw")
print(plot_climate_overview)
dev.off()

# Also save as EPS for vector graphics option
postscript(here("output", "plot_climate_overview.eps"), 
           width = 10, height = 6, horizontal = FALSE)
print(plot_climate_overview)
dev.off()

cat("High-resolution figures saved:\n")
cat("- plot_climate_overview.tiff (600 DPI)\n")
cat("- plot_climate_overview.eps (vector)\n")
```
...
df_climate_4_yearly_summary <- df_climate_2_merged %>%
  filter(Year > 2001 & Year < 2023) %>%
  dplyr::select(Year, Temperature, Precipitation) %>%
  arrange(Year)
